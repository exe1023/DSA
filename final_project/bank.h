...
private:
	...
	map<string , Account>::iterator LoginAccount;
...
public:
	...
	{
		map<string , Account>::iterator it = accounts.find(ID);
		if(it != accounts.end())
		{
	...
	void transfer(const string &ID, const int &Money)
	{
		...
		if(it == accounts.end())
		{
			...
			map<string , Account>::iterator now;
			now = accounts.begin();
	...
	void merge(string ID ,string Password , string merged_ID , string merged_Password)
	{
		map<string , Account>::iterator it, merged_it;
		it = accounts.find(ID);
		merged_it = accounts.find(merged_ID);
		if( it == accounts.end() )
			printf("ID %s not found\n" , ID.c_str());
		else if( merged_it == accounts.end() )
			printf("ID %s not found\n" , merged_ID.c_str());
		else if(it->second.password != md5(Password))
